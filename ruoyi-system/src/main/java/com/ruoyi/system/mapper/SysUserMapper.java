package com.ruoyi.system.mapper;

import com.ruoyi.common.core.domain.entity.SysUser;
import com.ruoyi.system.enums.TaisuFactory;
import org.apache.ibatis.annotations.Param;

import java.util.Date;
import java.util.List;

/**
 * 用户表 数据层
 *
 * @author ruoyi
 */
public interface SysUserMapper {
    /**
     * 根据条件分页查询用户列表
     *
     * @param sysUser 用户信息
     * @return 用户信息集合信息
     */
    public List<SysUser> selectUserList(SysUser sysUser);


    public List<SysUser> selectUserListAll(SysUser sysUser);

    /**
     * 根据条件分页查询已配用户角色列表
     *
     * @param user 用户信息
     * @return 用户信息集合信息
     */
    public List<SysUser> selectAllocatedList(SysUser user);

    /**
     * 根据条件分页查询未分配用户角色列表
     *
     * @param user 用户信息
     * @return 用户信息集合信息
     */
    public List<SysUser> selectUnallocatedList(SysUser user);

    /**
     * 通过用户名查询用户
     *
     * @param userName 用户名
     * @return 用户对象信息
     */
    public SysUser selectUserByUserName(String userName);

    /**
     * 通过身份证查询用户
     *
     * @param idCard 身份证
     * @return 用户对象信息
     */
    public SysUser selectUserByIdCard(String idCard);

    /**
     * 通过用户ID查询用户
     *
     * @param userId 用户ID
     * @return 用户对象信息
     */
    public SysUser selectUserById(Long userId);

    /**
     * 新增用户信息
     *
     * @param user 用户信息
     * @return 结果
     */
    public int insertUser(SysUser user);

    /**
     * 修改用户信息
     *
     * @param user 用户信息
     * @return 结果
     */
    public int updateUser(SysUser user);

    /**
     * 修改用户头像
     *
     * @param userName 用户名
     * @param avatar   头像地址
     * @return 结果
     */
    public int updateUserAvatar(@Param("userName") String userName, @Param("avatar") String avatar);

    /**
     * 重置用户密码
     *
     * @param userName 用户名
     * @param password 密码
     * @return 结果
     */
    public int resetUserPwd(@Param("userName") String userName, @Param("password") String password);

    /**
     * 通过用户ID删除用户
     *
     * @param userId 用户ID
     * @return 结果
     */
    public int deleteUserById(Long userId);

    /**
     * 批量删除用户信息
     *
     * @param userIds 需要删除的用户ID
     * @return 结果
     */
    public int deleteUserByIds(Long[] userIds);

    /**
     * 校验用户名称是否唯一
     *
     * @param userName 用户名称
     * @return 结果
     */
    public int checkUserNameUnique(String userName);

    /**
     * 校验手机号码是否唯一
     *
     * @param phonenumber 手机号码
     * @return 结果
     */
    public SysUser checkPhoneUnique(String phonenumber);

    /**
     * 校验email是否唯一
     *
     * @param email 用户邮箱
     * @return 结果
     */
    public SysUser checkEmailUnique(String email);

    /**
     * 根据员工的编号，获得员工对象。注意员工编号设置成要唯一。
     */
    public SysUser getByUserNo(String no);


    /**
     * 整合查询，传入参数则生效，null则忽略条件
     *
     * @param idCard         身份证号
     * @param locationCardNo 定位卡编号
     * @param carCardNo      车卡编号
     * @param plateNo        车牌号
     */
    public List<SysUser> getByCommonParams(@Param("idCard") String idCard,
                                           @Param("locationCardNo") String locationCardNo,
                                           @Param("carCardNo") String carCardNo,
                                           @Param("plateNo") String plateNo);

    /**
     * 下发成功后加个标志
     */
    void sendBackStatus(@Param("userId") Long userId, @Param("sendedStatus") Integer sendedStatus);

    int deleteFaceByUserId(@Param("userId") Long userId);

    List<SysUser> selectByEmpNo(@Param("empNo") String empNo);

    /**
     * 根据身份证获取
     *
     * @param idCards 身份证号
     */
    List<SysUser> selectListFaceByIdCards(@Param("idCards")List<String> idCards);

    /**
     * 根据身份证更新头像
     * @param face
     * @param idCard
     */
    void updateFaceByIdCard(@Param("face") String face,@Param("idCard") String idCard);

    List<SysUser> selectSysuserDoSynchronized(@Param("time") Date time);

    void updateSysuserDoSynchronized(@Param("time") Date time);


    List<SysUser> selectUserUnSync();

    /**
     * 根据身份证和厂区名称,修改员工对应厂区的同步状态
     * @param factory 厂区名称
     * @param idCard  身份证
     */
    void updateSyncedUser(@Param("idCard") String idCard,@Param("factory") TaisuFactory factory);

}
